...
1. **Distância do A3** — o que muda a reunião de 18/09 vem primeiro
2. **Densidade** — aulões (10k+ palavras) rendem mais por sessão, mas só depois da base aplicada
3. **Validação de audiência** — views altas indicam o que o mercado considera o melhor material dela
4. **Trava do trimestre** — prospecção ativa está SUSPENSA (decisão 23/08). Material de prospecção é preparo, não execução: prioridade média

⚠️ **Regra:** cada lote vira uma seção do `foundation/sales-playbook.md`. Nunca destilar sem escrever no playbook — transcrição lida e não destilada é trabalho perdido.
