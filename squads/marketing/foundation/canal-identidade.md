# Identidade do Canal — Impulso IA

> Criado 2026-08-25. **Decisões fechadas pelo Emerson.** Este arquivo é a fonte única do que vai colado no YouTube Studio.
> Não reabre nicho/público/tom (ver `creator-profile.md`, `mapa-teste-publico.md`, `brand-voice.md`).
> Checklist de execução: [`youtube-channel-setup.md`](./youtube-channel-setup.md)

---

## Decisões fechadas (25/08/2026)

| Item | Decisão |
|---|---|
| **Nome da marca** | Impulso IA |
| **Handle YouTube** | ✅ **`@emerson.impulsoia`** — CRIADO 25/08. Mesmo handle do Instagram (marca unificada). `@impulsoia` não ficou |
| **Nome de exibição** | ⚠️ Está **`ImpulsoIA`** no canal criado. Decisão original era `Emerson \| Impulso IA` — ver nota abaixo |
| **Conta Google (dona do canal)** | `emerson.impulsoia@gmail.com` — já criada, dedicada |
| **Categoria** | Educação |
| **País / Idioma** | Brasil / Português (Brasil) |
| **Instagram já reservado** | `@emerson.impulsoia` |
| **E-mail de contato público** | `emerson.impulsoia@gmail.com` (mesmo da conta, por ora — ver nota abaixo) |
| **LinkedIn** | `linkedin.com/in/emerson-trindade-8001` — perfil pessoal, conta Hotmail |


### ⚠️ Estado real do canal (verificado 25/08/2026)

**URL:** https://www.youtube.com/@emerson.impulsoia — **canal criado e no ar.**

| Campo | Decidido | No canal hoje |
|---|---|---|
| Handle | `@impulsoia` | ✅ `@emerson.impulsoia` — fallback previsto, sem problema |
| Nome de exibição | `Emerson \| Impulso IA` | ⚠️ `ImpulsoIA` |

**Sobre o handle:** ficou o fallback. Não é perda — mesmo handle do YouTube e do Instagram significa marca unificada e uma coisa só pra falar em vídeo. **Manter.**

**Sobre o nome de exibição:** `ImpulsoIA` (junto, sem espaço) perde as duas coisas que sustentavam a decisão original — o nome real do Emerson, que é de onde vem a autoridade ("conversa de quem já fez", `brand-voice.md`), e a legibilidade da marca. Nome de exibição **é editável a qualquer momento**, sem afetar URL nem inscritos. Recomendação: trocar para **`Emerson | Impulso IA`** em YouTube Studio → Personalização → Informações básicas. Custo: 10 segundos.

**Verificado por print 25/08 (página do canal logada):**

| Item | Estado |
|---|---|
| Foto de perfil | ✅ **PRONTA** — foto real, fundo preto liso, camiseta lisa, luz lateral limpa, enquadramento cabeça-e-ombros. Passa no teste dos 48px. *Ressalva menor: sorriso largo destoa levemente do tom "direto e sem hype" — trocar só se houver outra sessão de fotos, não é gargalo* |
| Nome de exibição | ⚠️ `ImpulsoIA` — trocar para `Emerson \| Impulso IA` |
| Descrição | ❌ **VAZIA** — só o "Saiba mais sobre este canal" padrão. Texto pronto neste arquivo, é colar |
| Banner | ❌ **AUSENTE** — gerar com `data/assets/banner-canal.html` |
| Palavras-chave / links / playlist | ❌ Não configurados |
| Channel ID | ❌ Não coletado — trava o worker de analytics |

**Ordem de impacto:** descrição (o YouTube usa pra entender o canal) → banner → nome → palavras-chave/links → playlist → Channel ID.

### Por que "Emerson | Impulso IA" e não só a marca
A autoridade do canal vem de *quem já fez* (`brand-voice.md` — "conversa de quem já fez, com quem ainda não fez"). Nome real primeiro sustenta isso; a marca atrás dá para onde crescer. Falado em vídeo continua curto: "se inscreve no Impulso IA".

### ⚠️ Risco de marca — registrado, não bloqueante
Existe **IMPULSO DIGITAL TECNOLOGIA E INTELIGENCIA ARTIFICIAL LTDA** (CNPJ 60.475.559/0001-08, São Paulo/SP, CNAE consultoria em TI) — **mesmo CNAE do Emerson**.

- **Não impede** usar "Impulso IA" como nome de canal. Nome de canal não é nome empresarial nem marca registrada.
- **Impede potencialmente** um registro de marca no INPI: mesmo radical ("Impulso") + mesma classe (tecnologia/TI) é a hipótese clássica de indeferimento por colidência.
- **Ação:** consultar a base do INPI antes de gastar com abertura de CNPJ "Impulso IA". Item aberto no Finance (Lorenzo).
- **Nome do canal segue independente dessa decisão** — se o CNPJ vier com outro nome, o canal não precisa mudar.


### Nota — e-mail de contato vs. e-mail dono do canal
`emerson.impulsoia@gmail.com` é a conta **proprietária** do canal (acesso total: exclui vídeos, muda monetização, transfere o canal). Usá-la também como contato público na descrição é aceitável agora, mas concentra risco: o endereço que recebe spam e proposta é o mesmo que controla o canal.

**Quando separar:** ao registrar domínio próprio, criar `contato@` e trocar só na descrição — a conta dona continua a mesma. Enquanto isso, **ativar verificação em 2 etapas nessa conta Google** é o mínimo, já que ela é o único ponto de falha do canal.

---

## Descrição do canal — colar no YouTube Studio

> As **2 primeiras linhas** aparecem sem "ver mais". Elas carregam o eixo e a prova. Sem slogan.

```
IA aplicada a trabalho que já existe. Não é IA pra criar negócio novo — é IA pra tirar o trabalho manual de quem já trabalha.

Eu construo e opero automação com IA em cliente real: atendimento no WhatsApp, registro de pedido, conciliação com o sistema. Aqui eu mostro como faz, quanto custa e o que quebrou.

O que você vai encontrar:
• Atendimento no WhatsApp com IA — bot que atende, responde e registra pedido
• Automação de processo com n8n — conectar sistema e tirar a tarefa manual do meio
• Saber pedir certo pra IA — prompt como habilidade de trabalho, não truque
• Dados que o negócio já tem — extrair número de quem já vende

Sem promessa de faturamento. Sem "o futuro chegou". Passo a passo completo, inclusive o que deu errado.

Contato: emerson.impulsoia@gmail.com
```

**Pronto para colar.** Nenhum placeholder pendente.

---

## Palavras-chave do canal
*Configurações → Canal → Informações básicas → Palavras-chave. Vocabulário de `brand-voice.md` seção "Usar" — palavra do dia a dia, não jargão puro.*

```
automação com IA, IA para pequenos negócios, WhatsApp com IA, n8n, automação de atendimento, inteligência artificial para empresas, ChatGPT para trabalho, como usar IA no negócio, automação sem código, IA aplicada, prompt para trabalho, automação de processos
```

---

## Identidade visual — especificação

| Peça | Tamanho | Direção |
|---|---|---|
| **Foto de perfil** | 800×800px | **Foto real do Emerson**, não logo. Autoridade vem da pessoa. Fundo limpo, rosto ocupando ~60% do quadro (aparece a 48×48px na maioria das telas — logo abstrato some) |
| **Banner** | 2560×1440px · área segura 1546×423px | Texto **só** na área segura. Linha única: **"IA aplicada a trabalho que já existe"**. Sem foto de robô, sem cérebro azul, sem circuito — é o visual padrão do hype que estamos evitando |
| **Watermark** | 150×150px, fundo transparente | Versão mono do "Impulso IA" ou inicial. Aparece o vídeo todo — precisa funcionar pequeno e sem cor |

**Paleta:** escolher 2 cores + neutro e travar. Evitar o azul-ciano-degradê genérico de conteúdo de IA — a diferenciação visual é a mesma do texto: parecer real, não parecer futurista.


### Links do canal (Configurações → Canal → Informações básicas → Links)
| Ordem | Link | Observação |
|---|---|---|
| 1 | `linkedin.com/in/emerson-trindade-8001` | Perfil pessoal. ⚠️ Está numa conta Hotmail pessoal — **adicionar `emerson.impulsoia@gmail.com` como e-mail secundário no LinkedIn** para o perfil ser recuperável se o Hotmail cair. Não trocar o principal |
| 2 | `instagram.com/emerson.impulsoia` | Reservado, sem conteúdo ainda — reserva o espaço |

O primeiro link aparece sobreposto ao banner. Ordem importa.

---

## Template de descrição de vídeo — escrever 1 vez, reaplicar sempre

```
[1 linha: o problema que o vídeo resolve, na linguagem de quem tem o problema]

[2-3 linhas: o que a pessoa sai sabendo FAZER depois de assistir. Verbo no infinitivo.]

⏱️ CAPÍTULOS
00:00 [Hook]
00:00 [Bloco 2]
00:00 [Bloco 3]
00:00 [Demonstração]
00:00 [Fecho]

🔗 LINKS CITADOS NO VÍDEO
- [ferramenta/artigo mencionado]

📌 FONTES DOS NÚMEROS
- [obrigatório quando o vídeo cita dado — ver dores-ia-brasil-2026.md]

---
Eu sou Emerson. Construo automação com IA em negócio real e mostro aqui como faz — inclusive o que quebra.
Se você quer que eu mostre algo específico, comenta que eu leio.

📷 Instagram: @emerson.impulsoia
💼 LinkedIn: linkedin.com/in/emerson-trindade-8001
✉️ Contato: emerson.impulsoia@gmail.com

#automacao #inteligenciaartificial #n8n
```

**Regras do template:**
- Timestamps começando em `00:00` ligam capítulos automaticamente — sempre preencher.
- Bloco "FONTES DOS NÚMEROS" é obrigatório quando houver dado. Regra do STATE: nenhum número vai ao ar sem estar em `dores-ia-brasil-2026.md`. A fonte visível é o que sustenta o "sem hype".
- Máximo 3-5 hashtags. Mais que isso o YouTube ignora.
- Nunca CTA de urgência ("corre", "última chance") — `brand-voice.md`, seção Evitar.

---

## Template de tags padrão
*Base fixa + 3-5 específicas do vídeo.*

```
automação, inteligência artificial, IA, n8n, whatsapp business, automação de atendimento, ia para negócios, produtividade, emerson impulso ia
```

---

## Playlist inicial

- **Nome:** `IA sem enrolação` — coerente com o tom, não amarra a público nenhum (P1-P5 seguem em teste)
- **Descrição:** "Como aplicar IA no trabalho que você já faz. Passo a passo, com o que funciona e o que não funciona."
- Vídeo 1 entra nela **desde o upload** — playlist vazia não custa nada e evita retrabalho depois.

---

## Ordem de execução

1. ~~Criar conta Google dedicada~~ ✅ `emerson.impulsoia@gmail.com` já existe — logar com ela
2. Criar canal → nome de exibição `Emerson | Impulso IA`
3. Reivindicar `@impulsoia` em [youtube.com/handle](https://www.youtube.com/handle) — se tomado, `@impulsoia.br`
4. Colar descrição + palavras-chave (acima)
5. Subir foto e banner
6. Criar playlist "IA sem enrolação"
7. **Registrar o Channel ID no `memory/STATE.md`** — é o que destrava o worker de analytics

> **Channel ID:** Configurações → Canal → Configurações avançadas. Começa com `UC...`. Sem ele o `youtube-analytics.py` não tem o que coletar.
