...
**[OK] ACERVO DE VENDAS FECHADO (27/08) — a maior lacuna do repo deixou de existir:** a pedido do Emerson, o **catálogo inteiro da Carol Iasmim** (`@carol.iasmim`, 175k) foi baixado — **58 vídeos, 2025+2026**, totalizando **59 transcrições e ~271 mil palavras** no Commercial (antes: 1 transcrição, 15% lida). **4 métodos nomeados** destilados em `squads/commercial/foundation/sales-playbook.md`: **Contrapeso** (preço/desconto), **Lavagem Cerebral Antiobjeção**, **Alicerce** (7 etapas do processo) e **AIDAS**. Skill `vendas` reescrita. **Primeiro canal fixo do Commercial** no radar semanal.
**Impacto direto no marco A3 (18/09):** a âncora da conversa com o Araújo é **R$ 4.000** — a contingência de R$1.500-3.000 é rede de segurança, **não ponto de partida**. Desconto de 10% sobre margem de 30% entrega 1/3 do lucro. **Dever de casa antes da reunião:** diferenciais + custo invisível do Araújo + 3 moedas de troca.
**[!] Lacuna que permanece:** 51 das 58 transcrições ainda não destiladas (carisma, prospecção, autoridade, fechamento, gatilhos, atendimento, mentalidade). Estão baixadas; a skill instrui a **declarar isso** quando a pergunta cair fora das 7 já processadas — em vez de improvisar.
**Bug corrigido em `_core/youtube-fetch-video.py`:** IDs de YouTube que começam com `-` eram lidos como flag pelo argparse e **derrubavam o lote inteiro** (aconteceu 2× nesta sessão). Agora são convertidos em URL antes do parse. Testado.

**[!] COLETA AUTOMATICA DESLIGADA — DECISAO DO EMERSON (27/08):** **nenhuma transcrição é mais baixada sozinha.** Toda **segunda-feira 09:00** a tarefa `HIVE-YouTube-Intel` roda `_core/youtube-radar.py` e gera uma **lista** em `data/youtube-radar/YYYY-MM-DD.md` — título, data, duração e views de todos os canais, **sem baixar nada**. O Emerson escolhe os números; só o escolhido vira transcrição e depois entra no playbook/skill do squad dono. Skill nova: **`radar-youtube`**.
**Motivo:** transcrição baixada que ninguém lê vira acervo morto — havia **67 transcrições baixadas e nunca lidas** (Dev 25, Infra 42) contra ~40 destiladas. A escolha do que vira conhecimento passa a ser humana.
**[!] 2 defeitos encontrados no caminho:** (1) a tarefa agendada rodava **semanalmente** mas cada squad tinha cadência própria (marketing 7d, dev/operations 14d, infra 28d) — a única execução real (24/08) **pulou os 4 squads** por cadência não vencida, e todo o acervo veio de coleta manual em 23/08; (2) o `--flat-playlist` do yt-dlp **não traz `upload_date`** — a 1ª versão do radar listou vídeo de 3 meses atrás como novidade da semana. Corrigido com `--print` em lote (lê data/views sem baixar vídeo nem legenda).

**✅ ACERVO VIROU SKILL (27/08) — 6 skills criadas em `.claude/skills/`, uma por tema, roteadas ao squad dono:** `roteiro-youtube` (Marketing/M1) · `influencia-digital` (Marketing/M4) · `dados-verificados` (Marketing/M2) · `metas-performance` (Operations) · `vendas` (Commercial) · `modelo-de-negocios` (Intelligence). Antes disso o conhecimento vivia só em `foundation/`, que **só entra no contexto se alguém abrir o arquivo** — skill carrega sozinha.
**Regra de ouro instalada em 4 lugares (redundante de propósito):** (1) `CLAUDE.md` raiz § Roteamento; (2) `CLAUDE.md` de cada um dos 4 squads donos; (3) `skills/open-squad.md` passo 4, antes de adotar persona; (4) hook `userprompt-squad-routing.py`, que passou a **anunciar a skill obrigatória** junto com o squad detectado. Skills são **cumulativas** — precificar o Araújo aciona `vendas` + `modelo-de-negocios`.
**⚠️ Bug encontrado e corrigido no hook de roteamento:** as palavras-chave estavam **em inglês e com personas de outro projeto** (Victor/Maya/Owen, que não existem no HIVE). Na prática "roteiro", "meta", "vender", "precificar" **não disparavam nada** — o roteamento automático estava morto para o jeito como o Emerson escreve. Reescrito em PT-BR com as personas certas; 5 cenários testados, todos passam. Backup do original no scratchpad da sessão.
**Squads SEM skill (acervo bruto, não destilado):** Dev, Infra, CS, Product, Finance, Quality — o hook agora **avisa explicitamente** "não improvisar como se houvesse".

**Aplicado no Neotriad via API (27/08):** os **10 marcos do trimestre viraram tarefas** no Neotriad (janela 01/09 a 30/10) — C2/C3/C4 (conteúdo), A1–A6 (Araújo) e o gatilho "cobrar o dono se a rede não liberar" (01/09). Todas triáde **I**, 120min (30min o gatilho), descrição com o critério de verificação objetivo de cada marco. Antes de criar, a janela 27/08–31/10 foi varrida: **0 tarefas relacionadas** — nada duplicado. `_core/neotriad.py` ganhou escrita (`criar_tarefas`, `excluir_tarefas`) — era só-leitura até então.
Todos os 10 vinculados ao papel **EMPREENDEDOR** (`ae2604df…`, o que está em uso real — 77 tarefas; o segundo papel de mesmo nome tem zero). **Nenhuma pendência manual.**
**⚠️ 3 limites da API do Neotriad descobertos no processo (documentados no docstring do script):** (1) **não existe endpoint de metas nem de projetos** — só tarefas, compromissos, papéis e categorias; meta só se cria na interface, então os marcos são tarefas soltas, não filhas de uma meta; (2) POST/PUT/DELETE operam em **lote** (corpo é sempre lista, mesmo para 1 item) — a doc não diz e objeto solto dá 400; (3) **papel só grava no POST e o formato é `Papel:[{"id_papel":"<guid>"}]`** — lista de OBJETOS. Lista de GUIDs (`Papel:["<guid>"]`) devolve 201 e é **ignorada em silêncio**: a tarefa nasce sem papel e sem erro. No PUT nenhum formato funciona — para corrigir papel de tarefa existente, **recriar e apagar a antiga** (foi o que se fez aqui: a 1ª leva nasceu sem papel por causa desse formato).
**Volume do Neotriad verificado (27/08) — está saudável:** a janela 27/08–31/10 mostra 559 tarefas, mas **549 são recorrências** de rotina ("Resumo do dia", "Atividade Física", "Empreendimento Digital"…, ~65x cada). **As únicas 10 tarefas não-recorrentes do trimestre são exatamente os marcos criados hoje.** Não há inchaço de agenda. **Ressalva real que fica:** 100% das tarefas da janela curta estão na triáde "I" (alvo 70%) — se tudo é Importante, a triáde deixa de ordenar. Os marcos entraram como "I" também, o que é correto, mas reclassificar as recorrências (rotina raramente é "I") faria o painel voltar a informar.
